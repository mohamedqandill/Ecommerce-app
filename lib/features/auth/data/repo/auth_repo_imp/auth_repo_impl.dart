...
class AuthRepoImpl implements AuthRepo {
  ...
  Future<Either<RouteFailures, bool>> login(
    ...
    try {
      ...
      if (!loggedIn) {
        return Left(
            RouteInvalidCredintailFailures("Invalid username or password "));
      }
  ...
  @override
  Future<Either<RouteFailures, bool>> newPassword(
      String email, String password) async {
    try {
      bool loggedIn = await authRemoteDataSource.newPassword(email, password);
      if (!loggedIn) {
        return Left(
            RouteInvalidCredintailFailures("Invalid username or password "));
      }
  ...
  @override
  Future<Either<RouteFailures, bool>> forgetPassword(String email) async {
    try {
      bool loggedIn = await authRemoteDataSource.forgetPassword(email);
      if (!loggedIn) {
        return Left(
            RouteInvalidCredintailFailures("Invalid username or password "));
      }
  ...
  @override
  Future<Either<RouteFailures, bool>> resetCode(String code) async {
    try {
      bool loggedIn = await authRemoteDataSource.resetCode(code);
      if (!loggedIn) {
        return Left(
            RouteInvalidCredintailFailures("Invalid username or password "));
      }
